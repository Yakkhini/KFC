...
def fid_wrapper(num_features=2048, device="cuda"):
    ...
    wrapper_model.eval()
    return FID(
        num_features=num_features, feature_extractor=wrapper_model, device=device
    )
    ...
    with torch.no_grad():
        for x, y in testFID_loader:
            ...
            if args.dataset == "mnist" or args.dataset == "fashion":
                ...
                x_hat = (
                    decoder(z, y_onehot).view(x.size(0), 1, 32, 32).repeat(1, 3, 1, 1)
                )
            elif (
                args.dataset == "svhn"
                or args.dataset == "cifar10"
                or args.dataset == "lsun"
            ):
                x = x.cuda()
...
def caculate_ACC(args, task_ID, train_data, test_data, labels, decoder):
    ...
    print(
        "[CLS Epoch%2d]\t LCS/loss: %.3f \t CLS/r_loss: %.3f \t ACC: %.2f%% \t rACC: %.2f%%"
        % (
            epoch + 1,
            loss_log["CLS/loss"],
            loss_log["CLS/r_loss"],
    ...
    append_to_csv(
        [task_ID, loss_log["ACC/FN"], loss_log["ACC/FP"]],
