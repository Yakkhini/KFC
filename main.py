...
def main_LGLvKR_Fine(args):
    ...
    sys.stdout = Logger(
        os.path.join(args.modeldir, "{}_log_{}.txt".format(args.name, args.op))
    )
    # training
    time_cost = 0
    for task_i in range(args.tasks):
        ...
        if not os.path.exists(
            ...
            optimizer_CVAE = torch.optim.Adam(
                cvae.parameters(), lr=args.lr, betas=(0.9, 0.999)
            )
            ...
            for epoch in range(args.epochs):
                ...
                print(
                    "[VAE Epoch%2d]\t V/loss: %.3f\t V/loss_rec: %.3f\t V/loss_var: %.3f"
                    % (
                        epoch + 1,
                        loss_log["V/loss"],
                        loss_log["V/loss_rec"],
        ...
        if args.LPIPS:
            caculate_LPIPS(
                args, task_i, test_data, labels_per_task_test[task_i], cvae.decode
            )
            ...
            for epoch in range(args.epochs):
                ...
                for x, y in train_loader:
                    ...
                    if task_i == 0:
                        ...
                        mu_hat, logvar_hat = cvae.encode(
                            xRec_merged[: args.batch_size], y
                        )
                        ...
                        loss_aug = torch.dist(
                            xRec_merged[: args.batch_size], xPre_old, 2
                        ) / xPre_old.size(0)
...
if __name__ == "__main__":
    ...
    common.add_argument(
        ...
        type=str,
        default="mnist",
        help="dataset name & uses dataset mnist/svhn/fasion",
    )
    common.add_argument(
        "-data_dir", type=str, default="./dataset/mnist", help="data directory"
    )
    ...
    network.add_argument(
        "-feat_dim", type=int, default=32 * 32, help="input features dimension"
    )
    network.add_argument(
        "-latent_dim", type=int, default=2, help="latent variable dimension"
    )
    network.add_argument(
    ...
    train.add_argument(
        ...
        type=float,
        default=1.0,
        help="alpha parameter for variational loss",
    )
    train.add_argument(
        "-alpha_var_hat",
        type=float,
    ...
    train.add_argument(
        "-epochs", default=10, type=int, metavar="N", help="number of epochs"
    )
    ...
    train.add_argument(
        ...
        type=str,
        default="10tasks",
        help="the name of the temporary saved model",
    )
    train.add_argument(
        "-method",
        type=str,
        default="KFC",
        choices=["KFC_Fine", "KFC_joint", "LGLvKR", "KFC_noFC", "KFC_noKR"],
    )
    ...
    common.add_argument(
        ...
        default=False,
        action="store_true",
        help="Whether LPIPS needs to be calculated",
    )
    common.add_argument(
        "-fid",
        default=False,
    ...
    common.add_argument(
        ...
        default=False,
        action="store_true",
        help="Whether ACC needs to be calculated",
    )
    common.add_argument(
        "-generate",
        default=False,
    ...
    if args.method == "KFC_fine":
        ...
        args.modeldir = (
            model_dir + "0_KFC_jointTraining/" + "{}epoch".format(args.epochs)
        )
